  ...
  begin match r with
    ...
    | ERegExDist (s,ss,ill) ->
      if !use_lens_context then
        ...
        let ss = List.map ~f:(lens_putr lc converter) ss in
        exampled_atom_to_exampled_dnf_regex
          (EADist (rep_type,s,converter,ss,ill))
          ill
      else
        exampled_atom_to_exampled_dnf_regex
          (EADist (s,s,Lens.LensIdentity (Regex.RegExDist s),ss,ill))
          ill
  end
