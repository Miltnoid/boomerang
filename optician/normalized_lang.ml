...
type exampled_regex =
  ...
  | ERegExDist of Regex.t * string list * (int list list)
...
let rec exampled_regex_to_string (r:exampled_regex) : string =
  ...
  | ERegExDist (r,ss,ill) -> paren ((Regex.show r) ^ (bracket (
      String.concat
        ~sep:";"
...
type exampled_atom =
  | EADist of Regex.t * Regex.t * Lens.t * string list * int list list
...
type ordered_exampled_atom =
  | OEADist of Regex.t * Regex.t * Lens.t * string list
...
let rec compare_exampled_atoms (a1:exampled_atom) (a2:exampled_atom) :
  comparison =
    begin match (a1,a2) with
      | (EADist (r1,_,_,el1,_), EADist (r2,_,_,el2,_)) ->
        let cmp = Regex.compare r1 r2 in
...
let rec compare_ordered_exampled_atoms (a1:ordered_exampled_atom)
                                     ...
                                     : comparison =
    begin match (a1,a2) with
      | (OEADist (r1,_,_,el1), OEADist (r2,_,_,el2)) ->
    ...
    | (OEAStar _, OEADist _) -> 1
    | (OEADist _, OEAStar _) -> -1
    end 
...
let rec to_ordered_exampled_atom (a:exampled_atom) : ordered_exampled_atom =
  begin match a with
  | EADist (r,rorig,lmap,el,_) -> OEADist (r,rorig,lmap,el)
...
let rec compare_atoms (a1:atom) (a2:atom) : comparison =
  begin match (a1,a2) with
  | (ADist r1, ADist r2) -> Regex.compare r1 r2
  | (ADist  _, AStar         _) -> -1
  | (AStar         _, ADist  _) -> 1
