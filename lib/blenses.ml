  ...
  and bij ml = match ml.bij with
    ...
    | None   ->
      let b = match ml.desc with
          | Copy(r1)                  -> true
          | Disconnect(r1,r2,f1,f2)   -> Rx.is_singleton r1 && Rx.is_singleton r2
  ...
  and astype ml = match ml.astype with
    ...
    | None ->
        let ct = match ml.desc with
          | Copy r             -> Arx.mk_rx r
          | Disconnect (r, _, _, _)  -> Arx.mk_rx r
          ...
          | Interleave (_,rs1) -> Arx.mk_rx (Safelist.fold_left (fun acc (_,ri) -> Rx.mk_seq acc (Rx.mk_star ri)) Rx.epsilon rs1)
  ...
  and avtype ml = match ml.avtype with
    ...
    | None ->
        let at = match ml.desc with
          ...
          | DupFirst (ml, _, _, _, r2) -> Arx.mk_seq (avtype ml) (Arx.mk_rx r2)
          | DupSecond (_, _, _, r2, ml)    -> Arx.mk_seq (Arx.mk_rx r2) (avtype ml)
          | Partition (_,rs1)  -> Arx.mk_rx (Safelist.fold_left (fun acc (_,ri) -> Rx.mk_seq acc (Rx.mk_star ri)) Rx.epsilon rs1)
          | Interleave (_,rs1) -> Arx.mk_rx (Rx.mk_star (Safelist.fold_left (fun acc (_,ri) -> Rx.mk_alt acc ri) Rx.empty rs1))
          | Merge r            -> Arx.mk_rx r
          | Fiat ml            -> avtype ml
          | Permute (p, mls)   ->
              let _, _, s2, _, _ = p in
